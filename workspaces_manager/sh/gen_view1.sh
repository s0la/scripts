...
while read line;do
	...
	if [ $num == $current_num ]; then
			...
			window_ws_num[${#window_ws_num[*]}]=$num
	fi
...
for i in ${windows[*]}; do
	...
	if [ ${window_ws_num[$index]} -eq $current_desk ]; then
		store[${#store[*]}]="wmctrl -i -r $i -e 0,$((x + ${coords[$c_index]})),$((y + ${coords[$((c_index + 1))]})),${w%%.*},${h%%.*} && wmctrl -i -a $i"
	else
